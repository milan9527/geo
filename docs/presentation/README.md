...
Slide text is editable in PowerPoint. Diagrams have separate editable draw.io sources and English SVG/PNG exports. All eleven screenshots come from actual English production interfaces; none uses substituted text or synthetic data. Screenshot URLs, capture times and hashes are recorded in speaker notes and the [capture manifest](../screenshots/manifest.json). The [slide manifest](manifest.json) maps screenshots to slides.
...
CairoSVG needs the system Cairo library. The diagrams use an English sans-serif font; install Arial, Liberation Sans or Noto Sans for consistent text layout.
...
The verification script checks local documentation links, English project prose, editable diagram labels, screenshot hashes, embedded screenshot bytes, slide bounds and rendered native text. It also regenerates the first-slide preview. Inspect the rendered PDF visually after content or layout changes; automated checks do not prove the absence of every visual overlap.
