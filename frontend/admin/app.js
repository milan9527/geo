...
function renderX402Event(event) {
  ...
  return `
    <div class="x402-event ${escapeHtml(event.status)}">
      ...
      <div>
        <strong data-original-language>${escapeHtml(eventArticleTitle(event.articleId, event.articleSlug, event.articleTitle || event.articleSlug || I18N.text("付费机器内容")))}</strong>
